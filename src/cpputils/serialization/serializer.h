...
class Serializer {
 public:
  ...
  template <class T>
  static uint64_t Serialize(const std::shared_ptr<T> &ptr,
                            std::vector<std::byte> &binary) {
    return Serializer::Serialize(*ptr, binary);
  }
  ...
  template <class T>
  static uint64_t Deserialize(std::shared_ptr<T> &ptr,
                              const std::span<std::byte> &binary) {
    ptr = std::make_shared<T>();
    return Serializer::Deserialize(*ptr, binary);
  }
